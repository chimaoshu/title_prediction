...
class RoughSet:
    ...
    def Ind(self, B: list) -> list:
        ...
        for row_order, decision_attribute_value in list(enumerate(decision_attributes_values)):
            ...
            # 找到决策属性对应的下标，这里需要考虑多列、单列的情况
            try:
                index: int = decision_attributes_posible_values.index(decision_attribute_value.tolist())

            # 单列
            except:
                index: int = decision_attributes_posible_values.index(decision_attribute_value)
    ...
    def POS(self, C: list, D: list) -> set:
        ...
        传入：
            C:condition attribute 条件属性的列序号组成的list或者set
    ...
    def Red(self, C: list, D: list) -> list:
        ...
        while (True):
            ...
            for element in C_copy:

                # 为了优化加速使用了不简洁的代码

                # 未去掉的正域长度
                POS_len = len(self.POS(Red_C, D))

                # 元素是否在列中
                try:
                    element_index = Red_C.index(element)
                except ValueError:
                    continue

                # 去掉列后的正域长度
                Red_C.pop(element_index)
                new_POS_len = len(self.POS(Red_C, D))

                # 判断是否存在冗余
                if POS_len == new_POS_len:
                    has_reduction = True
                    break
    ...
    def auto_reduction(self) -> list:
        ...
        print("\n对应的关键词为：")
        for i in df.columns[list(result)]:
            print(i, end=' ')
...
if __name__ == "__main__":
    ...
    csv_path = "2_粗糙集\\作业题.csv"
    # csv_path = "2_粗糙集\\gwt.csv"
    # csv_path = "4_新编码数据生成训练集\\3_2020年新数据.data_and_target"
